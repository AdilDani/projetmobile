...
public class Repository {
    ...
    /** The vehicule assigned to a chauffeur (matched on conducteurId), else the first one. */
    public void getCurrentVehicule(final String chauffeurId, final RepoCallback<Vehicule> cb) {
        getVehicules(new RepoCallback<List<Vehicule>>() {
            @Override public void onResult(List<Vehicule> list) {
                ...
                if (!list.isEmpty()) cb.onResult(list.get(0)); else cb.onError("Aucun véhicule");
            }
            @Override public void onError(String m) { cb.onError(m); }
        });
    }
    ...
    private <T> void enqueueObject(Call<T> call, final RepoCallback<T> cb) {
        call.enqueue(new Callback<T>() {
            @Override public void onResponse(Call<T> c, Response<T> r) {
                if (r.isSuccessful()) cb.onResult(r.body());
    ...
    private void enqueueVoid(Call<Void> call, final RepoCallback<Void> cb) {
        call.enqueue(new Callback<Void>() {
            @Override public void onResponse(Call<Void> c, Response<Void> r) {
                if (r.isSuccessful()) cb.onResult(null);
